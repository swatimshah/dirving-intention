...
summed_epochs = numpy.append(summed_epochs, orig_epochs[:, -1].reshape(len(my_epochs[:, -1]), 1), axis=1)
print(my_epochs[:, -1])
...
# Create the model
model=Sequential()
model.add(Conv1D(filters=22, kernel_size=4, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-1, max_value=1), padding='valid', activation='relu', strides=1, input_shape=(63, 63)))
